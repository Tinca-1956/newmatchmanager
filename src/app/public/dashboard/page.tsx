...
export default function PublicDashboardPage() {
  ...
  // In a public dashboard, we might want to show data from a default/featured club.
  // For now, let's assume we'll show data from the first club we find or a specific one.
  // This will be replaced with a club selector later.
  useEffect(() => {
    if (!firestore) {
        setIsLoading(false);
    ...
    const processMatches = async () => {
        ...
        // Fetch all matches for now. This should be refined to a single club.
        const allMatchesQuery = query(
            collection(firestore, 'matches'),
            orderBy('date', 'desc'),
            limit(20) // Limit the initial fetch
        );
        ...
        // No status updates on public page for performance
        ...
        <div className="flex flex-col gap-8">
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    ...
                    Welcome to Match Manager. Check out the latest activity.
                    </p>
                </div>
                <Button asChild>
